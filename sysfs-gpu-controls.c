...
float sysfs_gpu_temp(int gpu)
{
	...
	return(strtof(TempBuf, NULL) / 1000.0);
}

int fanpercent_to_speed(int gpu, int percent)
{
	int min = gpus[gpu].sysfs_info.MinFanSpeed;
	int max = gpus[gpu].sysfs_info.MaxFanSpeed;

	double range = max - min;
	double res = ((double)percent / 100.0) * range;
	res = round(res);
	return((int)res);
}

int fanspeed_to_percent(int gpu, int speed)
{
	int min = gpus[gpu].sysfs_info.MinFanSpeed;
	int max = gpus[gpu].sysfs_info.MaxFanSpeed;

	double range = max - min;
	double res = ((double)speed / range) * 100.0;
	res = round(res);
	return((int)res);
}

int sysfs_gpu_fanpercent(int gpu)
{
	char PWM1FileName[513], SpeedStr[33];
	int BytesRead, Percent;
	...
	Percent = strtoul(SpeedStr, NULL, 10);
	Percent = fanspeed_to_percent(gpu, Percent);
	
	return(Percent);
}

int sysfs_set_fanspeed(int gpu, int iFanSpeed)
{
	char PWM1FileName[513], Setting[33];
	int speed = fanpercent_to_speed(gpu, iFanSpeed);
	
	gpus[gpu].sysfs_info.TgtFanSpeed = iFanSpeed;
	
	snprintf(PWM1FileName, 512, "%s/pwm1", gpus[gpu].sysfs_info.HWMonPath);
	snprintf(Setting, 32, "%d", speed);
	
	if(WriteSysFSFile(Setting, PWM1FileName, strlen(Setting)) > 0) return(0);
	else return(-1);
}
 ...
 */
static bool sysfs_fan_autotune(int gpu, int temp, int fanpercent, int lasttemp, bool *fan_window)
{
...
{
	bool dummy;
	uint32_t Temp = (uint32_t)sysfs_gpu_temp(gpu);
